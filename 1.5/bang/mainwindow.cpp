...
{
    // 通过鼠标的hover确定落子的标记
    int xCoord = event->position().x();
    int yCoord = event->position().y();

    // 棋盘边缘不能落子
    if (xCoord >= kBoardMargin + kBlockSize / 2 &&
        xCoord < size().width() - kBoardMargin &&
        yCoord >= kBoardMargin + kBlockSize / 2 &&
        yCoord < size().height()- kBoardMargin)
    {
        // 获取最近的左上角的点
        int col = xCoord / kBlockSize;
        int row = yCoord / kBlockSize;
        ...
        len = sqrt((xCoord - leftTopPosX) * (xCoord - leftTopPosX) + (yCoord - leftTopPosY) * (yCoord - leftTopPosY));
        if (len < kPosDelta)
        {
            clickPosRow = row;
            clickPosCol = col;
        }
        len = sqrt((xCoord - leftTopPosX - kBlockSize) * (xCoord - leftTopPosX - kBlockSize) + (yCoord - leftTopPosY) * (yCoord - leftTopPosY));
        if (len < kPosDelta)
        {
            clickPosRow = row;
            clickPosCol = col + 1;
        }
        len = sqrt((xCoord - leftTopPosX) * (xCoord - leftTopPosX) + (yCoord - leftTopPosY - kBlockSize) * (yCoord - leftTopPosY - kBlockSize));
        if (len < kPosDelta)
        {
            clickPosRow = row + 1;
            clickPosCol = col;
        }
        len = sqrt((xCoord - leftTopPosX - kBlockSize) * (xCoord - leftTopPosX - kBlockSize) + (yCoord - leftTopPosY - kBlockSize) * (yCoord - leftTopPosY - kBlockSize));
        if (len < kPosDelta)
        {
            clickPosRow = row + 1;
            clickPosCol = col + 1;
